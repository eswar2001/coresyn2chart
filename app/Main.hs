...
main = do
  ...
  print ("found " <> show (length jsonFiles) <> " files")
  ...
  case functionName of
    "all" ->
      mapM_ (\(name,functionData) -> do
        ...
        print ("got " <> show paths <> " paths for the function: " <> name)
      ) (HM.toList hmBinds)
    x ->
      case HM.lookup x hmBinds of
        Just bind -> do
          ...
          paths <- readMVar pathsMvar
          print ("got " <> show paths <>  " paths for the function: " <> x)
...
processDumpFiles file prefixPath = do
  ...
  case eitherDecodeStrict content of
    Right (binds :: [LBind]) ->
      mapM (\functionData@(Function _name _type _ _) ->
      pure $ (_name,functionData)
      ) (translateCoreProgramToCFG binds)
    Left err -> do
      print err
      print file
      pure []
...
convertBindToEdgesList prefixPath root@(Function pName pType pIsCase pChildren) hmBinds mvar =
